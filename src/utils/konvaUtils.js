...
Konva.Image.prototype.cropReset = function (context) {
  // console.log("test", this.toDataURL())
  if (this.cropImage) {
    this.cropEnd()
  }
...
Konva.Image.prototype.cropStart = function (context) {
  ...
  // console.log('this.getStage', this.getStage);
  ...
  layer.add(this.cropImageTransformer)
...
export const cropFromComponent = () => {
  that.rotate(180)
  return false

  const image64 = that.cropEnd()

  var arr = image64.split(","),
    mime = arr[0].match(/:(.*?);/)[1],
    bstr = atob(arr[1]),
    n = bstr.length,
  ...
  const new_file = new File([u8arr], "order_image", { type: mime })
